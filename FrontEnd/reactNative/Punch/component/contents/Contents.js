...
import { Text, View, StyleSheet, TouchableOpacity, SafeAreaView, ScrollView, Button } from 'react-native';
// 컴포넌트 연결
  ...
  .catch( error => {
    ...
    render() {
      ...
      if(this.state.classList == null){
        list = <Text>수업이 없습니다. QR코드를 통해 수업을 등록하세요 </Text>
      }
      else if(this.state.classList == 'load'){
        list = <Text>수업을 불러오고 있습니다.</Text>
        this.classLoad()
      }
      else{
